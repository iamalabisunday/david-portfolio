...
import illustrations, { Book } from "../data/illustration";

export default function Publish({ id }: { id: number }) {
  const router = useRouter();

  // Find the current book using the ID
  const currentIndex = illustrations.findIndex((book) => book.id === id);
  ...
  const currentBook = illustrations[currentIndex];

  // Handlers for navigation
  const handleNext = () => {
    const nextIndex = (currentIndex + 1) % illustrations.length;
    const nextBook = illustrations[nextIndex];
  ...
  const handlePrevious = () => {
    ...
    const prevBook = illustrations[prevIndex];
